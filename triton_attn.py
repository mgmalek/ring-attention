"""
This file is modified from Tri Dao's Flash Attention implementation at:
...
def _flash_attn_forward(q, k, v, o, M, lse, tmp, is_final_iter, bias=None, causal=False, softmax_scale=None, mask_diagonal=False):
    # shape constraints
    ...
    assert bias is None, "bias is not currently supported"
    ...
    _, seqlen_block_k, _, _ = k.shape
    _, _, seqlen_q_rounded = M.shape

    assert k.shape == (batch, seqlen_block_k, nheads, d)
    assert v.shape == (batch, seqlen_block_k, nheads, d)

    if mask_diagonal:
        assert causal, (causal, mask_diagonal)
    ...
    _fwd_kernel[grid](
        ...
        *bias_strides,
        ...
        d,
        seqlen_q // 32,
        seqlen_block_k // 32,  # key for triton cache (limit number of compilations)
...
def _flash_attn_backward_prepro(do, o, lse):
    batch, seqlen_q, nheads, d = o.shape
...
def _flash_attn_backward(
    do, q, k, v, o, lse, delta, dq, dk, dv, bias=None, causal=False, softmax_scale=None, mask_diagonal=False,
):
    batch, seqlen_q, nheads, d = q.shape
    ...
    block_kv_seqlen = k.shape[1]
    assert k.shape == (batch, block_kv_seqlen, nheads, d)
    assert v.shape == (batch, block_kv_seqlen, nheads, d)

    if mask_diagonal:
        assert causal, (mask_diagonal, causal)

    grid = lambda META: (
        triton.cdiv(block_kv_seqlen, META["BLOCK_N"]) if META["SEQUENCE_PARALLEL"] else 1,
        batch * nheads,
    )
    _bwd_kernel[grid](
        q,
        ...
        *bias_strides,
        ...
        d,
        seqlen_q // 32,
        block_kv_seqlen // 32,  # key for triton cache (limit number of compilations)
        # Can't use kwargs here because triton autotune expects key to be args, not kwargs
        # IS_CAUSAL=causal, BLOCK_HEADDIM=d,
        bias_type,
        causal,
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, bias=None, causal=False, softmax_scale=None):
        ...
        """
        assert all(t.dtype == torch.bfloat16 for t in (q, k, v)), (q.dtype, k.dtype, v.dtype)
        ...
        seqlen_q_rounded = math.ceil(seqlen_q / 128) * 128
        M = torch.full((batch, nheads, seqlen_q_rounded), fill_value=float("-Inf"), device=q.device, dtype=torch.float32)
        lse = torch.full((batch, nheads, seqlen_q_rounded), fill_value=float("-Inf"), device=q.device, dtype=torch.float32)
    ...
    def backward(ctx, do):
        ...
        delta = _flash_attn_backward_prepro(do, o, lse)
...
class RingAttnFunc(torch.autograd.Function):
    @staticmethod
    def get_block_causal_and_skip(causal, rank, kv_rank) -> Tuple[bool, bool]:
        block_causal = causal and (kv_rank == rank)
        skip = causal and (kv_rank > rank)
        return block_causal, skip
    ...
    def forward(ctx, q, k, v, n_ranks, rank, prev_rank, next_rank, bias=None, causal=False, softmax_scale=None):
        ...
        send_buffer = torch.stack((k, v), dim=0)
        recv_buffer = torch.empty_like(send_buffer)

        for i in range(n_ranks):
            is_final_iter = (i == n_ranks - 1)

            if not is_final_iter:
                # send and recv new (k, v) asynchronously
                # in the final iteration, each rank will process the keys and values for next_rank
                # which it will store for the backward pass. then, the backward pass will compute
                # process the keys and values in the reverse order of the forward pass. this avoids
                # a blocking communication step at the end of the backward pass since at the start of
                # the end of the final iteration, each rank will process its own dk and dv
                send_op = dist.P2POp(dist.isend, send_buffer, next_rank)
                recv_op = dist.P2POp(dist.irecv, recv_buffer, prev_rank)
                reqs = dist.batch_isend_irecv([send_op, recv_op])

            # perform attn
            curr_kv = send_buffer
            k, v = torch.unbind_copy(curr_kv, dim=0)  # copy so that we can overlap communication with kernel execution

            kv_rank = (rank - i + n_ranks) % n_ranks

            block_causal, skip = RingAttnFunc.get_block_causal_and_skip(causal, rank, kv_rank)

            if causal:
                is_final_kv_iter = kv_rank == 0
            else:
                is_final_kv_iter = is_final_iter
        ...
        # NOTE: at the end of the forward pass, k and v are the keys and values for next_rank
        k_next = k
        v_next = v
        ctx.save_for_backward(q, k_next, v_next, o, lse, bias)
        ctx.causal = causal
        ctx.n_ranks = n_ranks
        ctx.rank = rank
    ...
    def backward(ctx, do):
        ...
        q, k, v, o, lse, bias = ctx.saved_tensors
        assert not ctx.needs_input_grad[7], "FlashAttention does not support bias gradient yet"
        ...
        kv_send_buffer = torch.stack((k, v), dim=0)
        kv_recv_buffer = torch.empty_like(kv_send_buffer)
        
        # NOTE: we keep dk and dv in fp32 to avoid precision issues. in the triton kernel, dk and dv accumulate in
        # fp32 and are cast to bf16 at the end of the kernel, after iterating over all query blocks. however, in
        # ring attention we re-cast to bf16 at the end of every rank's computation, which loses precision.
        # this issue is also present in the forward pass since o is accumulated in fp32 inside the kernel and cast
        # to bf16 at the end of each kernel, but in unit tests this does not seem to introduce precsion issues
        # TODO: sbould ablate this in a training loop to see if the precision is OK
        dkv_send_buffer = torch.empty_like(kv_send_buffer, dtype=torch.float32)
        dkv_recv_buffer = torch.zeros_like(dkv_send_buffer)

        dq = torch.zeros_like(q)
        dkv = torch.empty_like(dkv_send_buffer)

        for i in range(n_ranks):
            is_first_iter = (i == 0)
            is_final_iter = (i == n_ranks - 1)

            k, v = torch.unbind_copy(kv_send_buffer, dim=0)  # copy so that we can overlap communication with kernel execution
            ...
            p2p_ops = []
            if not is_final_iter:
                p2p_ops.append(dist.P2POp(dist.isend, kv_send_buffer, prev_rank))
                p2p_ops.append(dist.P2POp(dist.irecv, kv_recv_buffer, next_rank))

            if not is_first_iter:
                p2p_ops.append(dist.P2POp(dist.isend, dkv_send_buffer, prev_rank))
                p2p_ops.append(dist.P2POp(dist.irecv, dkv_recv_buffer, next_rank))

            reqs = []
            if len(p2p_ops):
                reqs = dist.batch_isend_irecv(p2p_ops)

            kv_rank = (next_rank + i) % n_ranks
            block_causal, skip = RingAttnFunc.get_block_causal_and_skip(causal, rank, kv_rank)
        ...
        dkv_send_buffer = dkv_send_buffer.to(k.dtype)
        dk, dv = torch.unbind(dkv_send_buffer, dim=0)

        return dq, dk, dv, None, None, None, None, None, None, None


class StripedAttnFunc(torch.autograd.Function):
    @staticmethod
    def get_block_causal_and_skip(causal, rank, kv_rank) -> Tuple[bool, bool]:
        block_causal = causal and (kv_rank == rank)
        skip = causal and (kv_rank > rank)
    ...
    def backward(ctx, do):
        ...
        for i in range(n_ranks):
            ...
            k, v = torch.unbind_copy(kv_send_buffer, dim=0)
...
class StepAttnFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, n_ranks, rank, prev_rank, next_rank, bias=None, causal=False, softmax_scale=None):
        """
        q: (batch_size, seqlen_q, nheads, headdim)
    ...
    def backward(ctx, do):
        ...
        lse_flat = lse.view(2 * batch, seqlen_q_rounded, n_heads)
        delta_flat = _flash_attn_backward_prepro(do, o_flat, lse_flat)
        delta = delta_flat.view(2, batch, seqlen_q_rounded, n_heads)
...
step_attn_func = StepAttnFunc.apply
