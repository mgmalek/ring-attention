...
def _test_attn_perf(
    ...
    if test_type == PerfTestType.PROFILE:
        ...
        logging.info(f"{median_duration = :8.4f}")

        median_peak_mem_gb = median(peak_mem_gbs)
        logging.info(f"{median_peak_mem_gb = :8.4f}")

        queue.put(dict(rank=rank, median_duration=median_duration, median_peak_mem_gb=median_peak_mem_gb))
...
if __name__ == "__main__":
    ...
    parser.add_argument("test_type", choices=("timing", "profile"))
    ...
    test_config = dict(
        batch_size=2,
        dim_per_head=128,
        dim=SweepList((2048, 4096, 8192)),
        seq_len=SweepList((16384, 32768, 65536, 131072, 262144)),
        attn_type=SweepList((AttentionType.STRIPED, AttentionType.STEP)),
        dtype=torch.bfloat16,
        num_warmup_iters=4,
        num_active_iters=4,
        test_type=PerfTestType.TIMING,
